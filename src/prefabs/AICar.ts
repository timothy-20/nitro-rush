...
export function createAICar(world: World, scene: THREE.Scene, SP: number[][], id: number): Entity {
  const entity = world.createEntity();

  const mesh = makeCar(COLORS_HEX[id], false);
  scene.add(mesh);

  const p = SP[0], q = SP[1];
  const dx = q[0] - p[0], dz = q[1] - p[1], l = Math.sqrt(dx * dx + dz * dz) || 1;
  const nx = -dz / l, nz = dx / l;
  const col = id % 2 === 0 ? 1 : -1, row = Math.floor(id / 2);
  ...
  world.addComponent(entity, 'Vehicle', { boostGauge: 0, boostActive: 0, boostMax: 1, drifting: false, driftIntensity: 0, hp: 100, dead: false, exploding: false, turn: 0, isPlayer: false, id, color: COLORS_CSS[id], name: NAMES[id] });
  world.addComponent(entity, 'AIDriver', { lookahead: 14 + id * 5, enabled: true });
